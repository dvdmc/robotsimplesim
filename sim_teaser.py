...
from dynamics import BasicVelocityDynamicModel, RoughnessTerrainDynamicModel
...
def simulation(exp_name, planner_name, seed):
    ...
    if 'greedy' in planner_name:
        planner1 = GreedyPlanner(agent1, low_level_planner1, list(beliefs.values()), 100, env, VISUALIZE_PLANNER)
    elif 'mcts' in planner_name:
        planner1 = MonteCarloTreeSearchPlanner(agent1, low_level_planner1, list(beliefs.values()), 100, env)
    else :
    ...
    if 'semantic' in exp_name:
        ...
        names.append('error')
        names.append('nees')
        names.append('consistency_ratio')
        names.append('nominal_consistency')
        names.append('rmse')
    ...
    # For nees we need to accumulate the errors and uncertainties for all targets
    errors_xy_single = []
    P_single = []
    nees_per_target = []
    avg_conf_per_target = []
    ...
    if 'semantic' in exp_name:
        ...
        nees = errors_xy_single.T @ np.linalg.inv(P_single) @ errors_xy_single
        ...
        chi_squared_nominal = chi2.ppf(0.95, 2)
        nominal_consistency = np.sum(nees_per_target) / (len(targets) * chi_squared_nominal)
        ...
        logger.addData(nees, 'nees')
        logger.addData(consistency_ratio, 'consistency_ratio')
        logger.addData(nominal_consistency, 'nominal_consistency')
    ...
    while planner1.budget > 0:
        ...
        # Gather errors for all beliefs and save the sum and the RMSE
        errors = []
        errors_xy = []
        errors_xy_single = []
        P_single = []
